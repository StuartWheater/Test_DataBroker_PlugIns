...
public class JSONRandomDataSourceFactory implements DataFlowNodeFactory
{
    public JSONRandomDataSourceFactory(String name, Map<String, String> properties)
    {
        _name       = name;
        ...
        throws InvalidClassException, InvalidMetaPropertyException, MissingMetaPropertyException
    {
        ...
        propertyNames.add(JSONRandomDataSource.TIMEINTERVAL_PROPERTYNAME);
    ...
    {
        if (dataFlowNodeClass.isAssignableFrom(JSONRandomDataSource.class))
            return (T) new JSONRandomDataSource(name, properties);
        else
            return null;
    }
