...
public class JSONFilterDataProcessorFactory implements DataFlowNodeFactory
{
    public JSONFilterDataProcessorFactory(String name, Map<String, String> properties)
    {
        _name       = name;
        ...
        throws InvalidClassException, InvalidMetaPropertyException, MissingMetaPropertyException
    {
        ...
        propertyNames.add(JSONFilterDataProcessor.REMOVEKEY_PROPERTYNAME);
    ...
    {
        if (dataFlowNodeClass.isAssignableFrom(JSONFilterDataProcessor.class))
            return (T) new JSONFilterDataProcessor(name, properties);
        else
            return null;
    }
